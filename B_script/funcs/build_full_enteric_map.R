############################################################################## #
#' Purpose: Map out completely cube to SQL relationships for Enterics
#' Author:  Michael Kuo
############################################################################## #
...
name_comparison <- 'enteric'
dir_output <- here::here('C_output', name_comparison)

catalogs <-
  c(
    ctrl = 'SA_PHRDW_Enteric_Panorama'
      ...
      match_words <-
        ...
        map(
          ~ fuzzy_match(
            ...
            dict    =
              list(
                'age_key' = c('age_at_surveillance_reported_date_key',
...
map_count_keys <-
  tribble(
    ...
    "Panorama Client Count",        'patient_master_key',
    "Panorama Disease Event Count", 'disease_event_id',   
      ...
      pmap_dfr(
        ...
        \(...) {
          ...
          if (nrow(match_hier) != 0) {
            ...
            # 1. Match with both dim and hier against vw_fact using cosine
            match_main <-
              fuzzy_match(
                a = 
                  dim_hier[2:3],
